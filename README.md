...
| `probes/test_gf64_gao_mateer.c` | Third independent cross-check of HQC 2026 TCHES Algorithm 2 (LCH14 addFFT) over GF(2^4), at a fresh affine shift `a = v_3 = basis[3]` outside `V_2`. Same algorithm as `test_lch14_variants.c` but independently retyped: forward-output match rate at n=2 and n=4 (16⁴ cases each), and convolution probe at n=4 with both length-2 inputs (65 536 cases) and length-4 inputs (16⁸ cases). Originally implemented hamil 2016 Algorithm 3.5; that port had Eq. 3.5 subtleties (Frobenius-closure on cosets) that were not safely verifiable locally, so the file is now a third implementation of Algorithm 2. |
